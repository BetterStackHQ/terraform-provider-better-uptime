...
# -gcflags "all=-N -l" is here for delve (`go tool compile -help` for more)
	go build -gcflags "all=-N -l" -ldflags "-X main.version=0.3.18"

install: build
	PLUGIN_DIR="$$HOME/.terraform.d/plugins/registry.terraform.io/BetterStackHQ/better-uptime/0.3.18/$$(go env GOOS)_$$(go env GOARCH)" && \
		mkdir -p "$$PLUGIN_DIR" && \
		cp terraform-provider-better-uptime "$$PLUGIN_DIR/"

uninstall:
	rm -rf "$$HOME/.terraform.d/plugins/registry.terraform.io/BetterStackHQ/better-uptime/0.3.18"
