...
resource "betteruptime_status_page_resource" "monitor_status" {
  ...
  # Show a response-times chart (also: plain, history, intraday_history)
...
resource "betteruptime_status_page_resource" "heartbeat" {
  ...
  # Help text shown next to the resource
  explanation = "Public API used by customer integrations"

  # Explicit position within the section, indexed from zero
  position = 2
}
...
# An Amazon SNS integration on the status page, marked down while it has an ongoing incident
resource "betteruptime_status_page_resource" "amazon_sns" {
  status_page_id         = betteruptime_status_page.this.id
  status_page_section_id = betteruptime_status_page_section.monitors.id
  resource_id            = betteruptime_amazon_sns_integration.backend_alerts.id
  resource_type          = "AmazonSnsIntegration"
